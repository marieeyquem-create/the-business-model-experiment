...
L'assistant conduit un echange progressif, apporte une premiere piste de travail, puis peut proposer au maximum deux actions provenant du site : fiche template, guide de combinaison, formation, coaching ou contact. Il peut aussi conclure qu'aucune offre n'est pertinente.

Le point d'entree principal est `Diagnostic de mon modele et de mon previsionnel`. L'assistant caracterise d'abord l'activite et ses sources de revenus. Si le visiteur possede deja un fichier Excel ou CSV, il peut ensuite l'envoyer facultativement. S'il n'en possede pas, la conversation se poursuit et aboutit a une recommandation de structure, de templates ou d'accompagnement.
...
Cette premiere lecture doit creer un declic sans remplacer l'offre payante. Elle selectionne au maximum quatre problemes, trois hypotheses importantes manquantes et trois priorites. Elle indique la direction a suivre, mais ne fournit ni modele complet, ni formules Excel, ni architecture detaillee feuille par feuille, ni mode d'emploi cellule par cellule. La recommandation commerciale explique sobrement quelle ressource permet de mettre les corrections en oeuvre. Elle n'est affichee que si elle est reellement pertinente.
